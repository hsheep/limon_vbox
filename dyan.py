...
class Virtualbox:
    ...
    def __init__(self, vm_guid):
        self.vm_guid = vm_guid
        self.xmlrpc = None
        self.dict_pid = {}

    def set_credentials(self, ip, port):
        print ("# connect %s:%s" % (ip, port))
        self.xmlrpc = xmlrpclib.ServerProxy('http://%s:%s' % (ip, port))

    def revert_current(self):
        print ("# revert snapshot guid: %s" % self.vm_guid)
        proc = subprocess.Popen(["VBoxManage", "snapshot", self.vm_guid, "restorecurrent"], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout

    def start(self):
        print ("# start vbox guid: %s" % self.vm_guid)
        proc = subprocess.Popen(["VBoxManage", "startvm", self.vm_guid, "--type", "headless"], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout

    def copytovm(self, src, dst):
        print ("# copy %s to %s" % (src, dst))
        return self.xmlrpc.copyto(xmlrpclib.Binary(open(src, "rb").read()), dst)

    def copyfromvm(self, src, dst):
        print ("# get vm file %s as %s" % (src, dst))
        data = self.xmlrpc.copyfrom(src)
        if data:
            open(dst, "wb+").write(data.data)
            if dst.endswith('.scap'):
                os.system("mv %s %s.gz;gunzip %s.gz" % (dst, dst, dst))
            return True
        else:
            return False

    def capturescreen(self, dst):
        proc = subprocess.Popen(["VBoxManage", "controlvm", self.vm_guid, "screenshotpng", dst], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout

    def suspend(self):
        proc = subprocess.Popen(["VBoxManage", "controlvm", self.vm_guid, "pause"], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout

    def get_vmmem(self):
        print ("# get vbox coredump")
        proc = subprocess.Popen(["VBoxManage", "debugvm", self.vm_guid, "dumpguestcore", "--filename", "%s.elf" % self.vm_guid], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout
        return "%s.elf"%self.vm_guid

    def stop(self):
        proc = subprocess.Popen(["VBoxManage", "controlvm", self.vm_guid, "poweroff"], stdout=subprocess.PIPE)
        vm_stdout = proc.communicate()[0]
        print vm_stdout

    # List directory in guest
    def list_dir(self, dir_name):
        print ("# get dir_list %s" % dir_name)
        return self.xmlrpc.listDirectoryInGuest(dir_name)
    ...
    def stop_sysdig(self):
        print ("# stop sysdig")
        for item in self.dict_pid:
            pid = self.dict_pid[item]
            try:
                self.xmlrpc.killProcessInGuest(pid)
            except Exception as e:
                print e

    def execute_file(self, mal_file, args):
        pid = self.xmlrpc.runProgramInGuest(mal_file, args)
        if pid:
            print ("execute_file pid: %s" % pid)
        else:
            print ("execute_file failed")

    def execute_sysdig(self, sysdig_file, cap_filter, cap_out_file, filter_file_name):
        cap_filter = cap_filter + " " + "and (proc.name=" + filter_file_name[:15] + " " + "or proc.aname=" + filter_file_name[:15] + ")"
        pid = self.xmlrpc.runProgramInGuest(sysdig_file, [cap_filter, "-z", "-w", cap_out_file])

        if pid:
            print ("execute_sysdig pid: %s" % pid)
            self.dict_pid[sysdig_file] = pid
        else:
            print ("execute_sysdig failed")

    def execute_sysdig_full(self, sysdig_file, cap_out_file, filter_file_name):
        cap_filter = "proc.name=" + filter_file_name[15] + " " + "or proc.aname=" + filter_file_name[:15]
        pid = self.xmlrpc.runProgramInGuest(sysdig_file, [cap_filter, "-z", "-w", cap_out_file])
        if pid:
            print ("execute_sysdig_full pid: %s" % pid)
            self.dict_pid[sysdig_file] = pid
        else:
            print ("execute_sysdig_full failed")

    def execute_strace(self, strace_path, strace_out_file, strace_filter, print_hexdump, mal_file, args):
        if print_hexdump:
            cmd = ["-o", strace_out_file, strace_filter, "-s", "64", "-eread=all", "-ewrite=all", "-f", mal_file]
        else:
            cmd = ["-o", strace_out_file, strace_filter, "-s", "216", "-f", mal_file]
        cmd.extend(args)
        pid = self.xmlrpc.runProgramInGuest(strace_path, cmd)
        if pid:
            print ("execute_strace pid: %s" % pid)
        else:
            print ("execute_strace failed")

    def execute_strace_full(self, strace_path, strace_out_file, print_hexdump, mal_file, args):
        if print_hexdump:
            cmd = ["-o", strace_out_file, "-s", "64", "-eread=all", "-ewrite=all", "-f", mal_file]
        else:
            cmd = ["-o", strace_out_file, "-s", "216", "-f", mal_file]
        cmd.extend(args)
        pid = self.xmlrpc.runProgramInGuest(strace_path, cmd)
        if pid:
            print ("execute_strace_full pid: %s" % pid)
        else:
            print ("execute_strace_full failed")

    def read_capture_and_dump(self, host_sysdig_path, capture_out_file, capture_out_txt_file, cap_format):
        print ("# read_capture_and_dump ")
        cap_format = '"' + cap_format + '"'
        cmd = host_sysdig_path + " " + "-p" + cap_format + " " + "-r" + " " + capture_out_file + " > " + capture_out_txt_file
        p = subprocess.Popen(cmd, shell=True)
        p.wait()
    ...
    def get_guest_logs(self):
        print self.xmlrpc.get_logs()
#######################################[end of virtualbox class]###################################
